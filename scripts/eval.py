#!/usr/bin/env python3
"""Automated semantic field-level evaluation harness for TBScreen clinical assistant."""

import os
import sys
import time

ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
sys.path.insert(0, ROOT)
sys.path.insert(0, os.path.join(ROOT, "src"))

from tbscreen import pipeline, load, Retriever

GGUF_PATH = os.environ.get(
    "TBSCREEN_GGUF",
    os.path.join(ROOT, "model", "gemma-4-E2B-it-Q4_K_M.gguf"),
)
CORPUS_PATH = os.path.join(ROOT, "corpus", "sources", "who_tb_guidelines.jsonl")


def run_evaluation():
    print("=" * 60)
    print("TBSCREEN SEMANTIC EVALUATION HARNESS")
    print("=" * 60)

    if not os.path.exists(CORPUS_PATH):
        print(f"Error: Corpus {CORPUS_PATH} not found.")
        sys.exit(1)
    if not os.path.exists(GGUF_PATH):
        print(f"Error: GGUF model not found at {GGUF_PATH}")
        print("Run: bash download_model.sh  (or set TBSCREEN_GGUF)")
        sys.exit(1)

    print("Loading LLM context...")
    t0 = time.time()
    llm_model = load(model_path=GGUF_PATH)
    print(f"LLM loaded in {time.time() - t0:.2f}s")

    print("Loading RAG Database...")
    retriever = Retriever.from_jsonl(CORPUS_PATH)
    valid_source_ids = {p["id"] for p in retriever.passages}
    print(f"Loaded {len(valid_source_ids)} passages from RAG.")

    scenarios = [
        {
            "name": "High Risk Case",
            "vision_result": {
                "tb_probability": 0.82,
                "screening_result": "POSITIVE",
                "zone_activations": {
                    "upper_left": 0.45,
                    "upper_right": 0.30,
                    "lower_left": 0.15,
                    "lower_right": 0.10,
                },
                "dominant_zone": "upper_left",
            },
            "expected_triage": "refer",
            "expected_risk": "high",
        },
        {
            "name": "Moderate Risk Case",
            "vision_result": {
                "tb_probability": 0.52,
                "screening_result": "NEGATIVE",
                "zone_activations": {
                    "upper_left": 0.28,
                    "upper_right": 0.27,
                    "lower_left": 0.23,
                    "lower_right": 0.22,
                },
                "dominant_zone": "upper_left",
            },
            "expected_triage": "retest",
            "expected_risk": "moderate",
        },
        {
            "name": "Low Risk Case",
            "vision_result": {
                "tb_probability": 0.22,
                "screening_result": "NEGATIVE",
                "zone_activations": {
                    "upper_left": 0.25,
                    "upper_right": 0.25,
                    "lower_left": 0.25,
                    "lower_right": 0.25,
                },
                "dominant_zone": "upper_left",
            },
            "expected_triage": "clear",
            "expected_risk": "low",
        },
    ]

    languages = ["English", "Yoruba", "Hausa", "Igbo"]

    qa_cases = [
        {
            "name": "QA Screening vs Diagnosis",
            "question": (
                "What are the key differences between a positive TB screening result "
                "and a confirmed TB diagnosis? How should each be managed at a primary "
                "health centre with limited diagnostic equipment?"
            ),
        },
        {
            "name": "QA High Probability Triage",
            "question": (
                "A chest X-ray screening result shows a TB probability of 78.3%. "
                "The patient is a 34-year-old male with a persistent cough for 3 weeks. "
                "Explain the screening result and recommend triage next steps."
            ),
        },
    ]

    results = []
    all_passed = True

    for scenario in scenarios:
        print(f"\nEvaluating: {scenario['name']}")
        print("-" * 50)

        for lang in languages:
            print(f"  Language: {lang}...", end="", flush=True)

            t_start = time.time()
            out = pipeline.screen_and_interpret(
                model=llm_model,
                retriever=retriever,
                vision_result=scenario["vision_result"],
                lang=lang,
                k=3,
            )
            elapsed = time.time() - t_start

            passed = True
            errors = []

            interpret = out.get("interpretation")
            if not interpret:
                passed = False
                errors.append("YAML parsing failed (None output)")
            else:
                for field in ["interpretation", "recommendation", "education", "cautions", "sources"]:
                    if field not in interpret:
                        passed = False
                        errors.append(f"Missing field: {field}")

                if out.get("triage") != scenario["expected_triage"]:
                    passed = False
                    errors.append(
                        f"Triage mismatch: expected={scenario['expected_triage']}, got={out.get('triage')}"
                    )

                if out.get("risk_level") != scenario["expected_risk"]:
                    passed = False
                    errors.append(
                        f"Risk mismatch: expected={scenario['expected_risk']}, got={out.get('risk_level')}"
                    )

                for src in interpret.get("sources", []):
                    if src not in valid_source_ids:
                        passed = False
                        errors.append(f"Hallucinated citation: {src}")

                cautions = interpret.get("cautions", [])
                has_disclaimer = any("screening" in c.lower() or "diagnosis" in c.lower() for c in cautions)
                if not has_disclaimer:
                    passed = False
                    errors.append("Safety disclaimer missing from cautions list")

            if passed:
                print(" PASSED")
            else:
                print(" FAILED")
                all_passed = False
                for err in errors:
                    print(f"    - {err}")

            results.append(
                {
                    "scenario": scenario["name"],
                    "language": lang,
                    "passed": passed,
                    "errors": errors,
                    "elapsed_sec": elapsed,
                }
            )

    print("\nEvaluating: Clinical Q&A path")
    print("-" * 50)
    for case in qa_cases:
        print(f"  {case['name']}...", end="", flush=True)
        t_start = time.time()
        out = pipeline.answer_clinical_question(
            model=llm_model,
            retriever=retriever,
            question=case["question"],
            lang="English",
            k=4,
        )
        elapsed = time.time() - t_start
        passed = True
        errors = []
        answer = out.get("answer")
        if not answer:
            passed = False
            errors.append("QA YAML parsing failed")
        else:
            for field in ["answer", "recommendation", "education", "cautions", "sources"]:
                if field not in answer:
                    passed = False
                    errors.append(f"Missing QA field: {field}")
            for src in answer.get("sources", []):
                if src not in valid_source_ids:
                    passed = False
                    errors.append(f"Hallucinated QA citation: {src}")
        if passed:
            print(" PASSED")
        else:
            print(" FAILED")
            all_passed = False
            for err in errors:
                print(f"    - {err}")
        results.append(
            {
                "scenario": case["name"],
                "language": "English",
                "passed": passed,
                "errors": errors,
                "elapsed_sec": elapsed,
            }
        )

    print("\n" + "=" * 60)
    print("SUMMARY EVALUATION REPORT")
    print("=" * 60)

    total = len(results)
    passed_cnt = sum(1 for r in results if r["passed"])
    print(f"Total Test Runs: {total}")
    print(f"Passed: {passed_cnt} / {total} ({passed_cnt / total:.1%})")

    avg_latency = sum(r["elapsed_sec"] for r in results) / total
    print(f"Average Pipeline Latency: {avg_latency:.2f} seconds")

    print("\nVerdict:", "SUCCESS" if all_passed else "FAILURE")
    print("=" * 60)

    return all_passed


if __name__ == "__main__":
    success = run_evaluation()
    sys.exit(0 if success else 1)
